"""
Staging settings for codea_auth_server project.

These settings are used when ENVIRONMENT='staging'.
"""
...
# In staging, set DJANGO_SECRET_KEY environment variable
...
CORS_ALLOW_ALL_ORIGINS = False  # Never allow all origins in staging/production
...
# All values MUST be set via environment variables in staging
...
if not GOOGLE_OAUTH_CONFIG['CLIENT_ID']:
    raise ValueError("GOOGLE_CLIENT_ID environment variable must be set in staging!")
if not GOOGLE_OAUTH_CONFIG['CLIENT_SECRET']:
    raise ValueError("GOOGLE_CLIENT_SECRET environment variable must be set in staging!")
if not GOOGLE_OAUTH_CONFIG['REDIRECT_URI']:
    raise ValueError("GOOGLE_REDIRECT_URI environment variable must be set in staging!")

# Google OAuth Token Encryption Key
# MUST be set via environment variable in staging
...
LOGGING = {
    ...
    'loggers': {
        ...
        'django.db.backends': {
            ...
            'level': 'WARNING',  # Reduce DB query logging in staging
            'propagate': False,
        },
        'codea_auth_server': {
